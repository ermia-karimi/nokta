...
function getWords(text) {
    return normalize(text).split(" ").filter(w => w.length > 2 && !stopWords.includes(w));
}

// الگوریتم ساده TF با امتیازدهی و وزن‌بندی
function scoreSentences(sentences) {
    const allWords = getWords(sentences.join(" "));
    const freq = {};
    allWords.forEach(w => freq[w] = (freq[w] || 0) + 1);
...
function render(sentences, selectedSet, sentenceLevels) {
    content.innerHTML = sentences.map(s => {
        if (selectedSet.has(s)) {
            let cls = sentenceLevels[s];
            return `<div class="sentence highlight ${cls}">${s}</div>`;
        }
        return `<div class="sentence">${s}</div>`;
    }).join("");
...
function goBack() { highlightPage.style.display = "none"; mainPage.style.display = "block"; }
